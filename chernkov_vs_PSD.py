...
outdir  = "/lustre/research/hep/akshriva/Dream-testbeam2-analysis/Cherenkov_vs_PSD_plots_2D_linear/"
...
# --- Plot: 3 subplots (one per Cherenkov counter) ---
def plot_cherenkov_vs_psd(run_number, beam_energy, beam_type, psd_ints, chr1, chr2, chr3):
    """Create one figure with 3 subplots: Cherenkov1–3 vs PSD integrals (2D histograms)."""
    ...
    for ax, (ch, label) in zip(axes, ch_data):
        #h = ax.hist2d(ch, psd_ints, bins=150, cmap='viridis', norm=LogNorm(), cmin=1)
        h = ax.hist2d(ch, psd_ints, bins=150, cmap='viridis', cmin=1)
        ax.set_xlabel(f"{label} Integral (baseline-subtracted)")
        ax.set_xlim(0, 40000)
        ax.set_ylim(0, 40000)
        ax.grid(alpha=0.3)
        ax.set_title(label)
    
    axes[0].set_ylabel("PSD Integral (baseline-subtracted)")
    fig.suptitle(f"Cherenkov vs PSD Integrals\nRun {run_number}, {beam_energy} GeV {beam_type}", fontsize=14)
    plt.tight_layout(rect=[0, 0, 1, 0.95])

    # Shared colorbar for all subplots
    cbar = fig.colorbar(h[3], ax=axes.ravel().tolist(), label='Event Count (log scale)')

    out_file = os.path.join(
        outdir, f"Cherenkov_vs_PSD_run{run_number}_{beam_energy}GeV_{beam_type}.pdf"
    )
    plt.savefig(out_file)
    plt.close()
    print(f"✅ Saved multi-panel Cherenkov–PSD plot → {out_file}")

# --- Analysis function ---
def analyze_cherenkov_vs_psd(file_path, run_number, beam_energy, beam_type):
    """Load ROOT file, compute integrals for PSD and 3 Cherenkov counters, and plot."""
    ...
    print(f"🔹 Loaded {len(psd_data)} events from {file_path}")

    psd_ints  = integrate_waveforms(psd_data)
    chr1_ints = integrate_waveforms(chr1_data)
    chr2_ints = integrate_waveforms(chr2_data)
    chr3_ints = integrate_waveforms(chr3_data)

    plot_cherenkov_vs_psd(run_number, beam_energy, beam_type, psd_ints, chr1_ints, chr2_ints, chr3_ints)
...
def run_all():
    ...
    for fname, energy in positron_files.items():
        ...
        analyze_cherenkov_vs_psd(os.path.join(basedir, fname), run_number, energy, "positron")

    print("=== Processing pion files ===")
    for fname, energy in pion_files.items():
        run_number = fname.split('_')[0].replace("run", "")
        analyze_cherenkov_vs_psd(os.path.join(basedir, fname), run_number, energy, "pion")

    print("=== Processing muon files ===")
    for fname, energy in muon_files.items():
        run_number = fname.split('_')[0].replace("run", "")
        analyze_cherenkov_vs_psd(os.path.join(basedir, fname), run_number, energy, "muon")
